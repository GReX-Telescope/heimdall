...
hd_error hd_execute(hd_pipeline pl, const hd_byte *h_filterbank, hd_size nsamps,
  ...
  error = clean_filterbank_rfi(
      ...
      &pl->h_clean_filterbank[0], &h_killmask[0], cleaning_dm, pl->params.dt,
      pl->params.baseline_length, pl->params.rfi_tol, pl->params.rfi_min_beams,
      pl->params.rfi_broad, pl->params.rfi_narrow,
  ...
  *nsamps_processed = nsamps_computed - pl->params.boxcar_max;
  ...
  for (hd_size dm_idx = 0; dm_idx < dm_count; ++dm_idx) {
    ...
    for (hd_size filter_width = cur_dm_scrunch;
      ...
      if (pl->params.verbosity >= 4) {
        cout << "Filtering each beam at width of " << filter_width
             << " filter_idx=" << filter_idx << endl;
      }
      ...
      if (pl->params.boxcar_renorm) {
        ...
        // rescaling. Note that this method reduces the S/N of injected pulses.
        ...
        thrust::transform(
            ...
            norm_val_iter, thrust::device_ptr<hd_float>(filtered_series),
            thrust::multiplies<hd_float>());
      }
      ...
      if (pl->params.verbosity >= 4) {
        cerr << "pl->params.cand_sep_time=" << pl->params.cand_sep_time
             << " rel_rel_filter_width=" << rel_rel_filter_width << endl;
      }
      ...
      if (pl->params.max_giant_rate &&
          (total_giant_count / data_length_mins > pl->params.max_giant_rate)) {
        too_many_giants = true;
        float searched = ((float)dm_idx * 100) / (float)dm_count;
        cout << "WARNING: exceeded max giants/min, DM [" << dm_list[dm_idx]
