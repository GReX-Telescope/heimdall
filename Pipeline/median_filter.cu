...
inline __host__ __device__ float median3(float a, float b, float c) {
  return a < b ? b < c ? b : a < c ? c : a : a < c ? a : b < c ? c : b;
}
inline __host__ __device__ float median4(float a, float b, float c, float d) {
...
inline __host__ __device__ float median5(float a, float b, float c, float d,
                                         float e) {
...
hd_error median_scrunch5(const hd_float *d_in, hd_size count, hd_float *d_out) {
  ...
  if (count == 1) {
    ...
    *d_out_begin = median3(d_in_begin[0], d_in_begin[1], d_in_begin[2]);
  } else if (count == 4) {
    *d_out_begin =
        median4(d_in_begin[0], d_in_begin[1], d_in_begin[2], d_in_begin[3]);
  } else {
    // Note: Truncating here is necessary
...
hd_error median_scrunch3_array(const hd_float *d_in, hd_size array_size,
                               hd_size count, hd_float *d_out) {
  thrust::device_ptr<hd_float> d_out_begin(d_out);
  // Note: Truncating here is necessary
  hd_size out_count = count / 3;
...
hd_error median_scrunch5_array(const hd_float *d_in, hd_size array_size,
                               hd_size count, hd_float *d_out) {
  thrust::device_ptr<hd_float> d_out_begin(d_out);
  // Note: Truncating here is necessary
  hd_size out_count = count / 5;
...
hd_error mean_scrunch2_array(const hd_float *d_in, hd_size array_size,
                             hd_size count, hd_float *d_out) {
  thrust::device_ptr<hd_float> d_out_begin(d_out);
  // Note: Truncating here is necessary
...
struct linear_stretch_functor2
  ...
  linear_stretch_functor2(const hd_float *in_, unsigned in_size, float step)
      : in(in_), in_size(in_size), step(step),
        correction(((int)(step / 2)) / step) {}

  inline __host__ __device__ hd_float operator()(unsigned out_idx) const {
...
struct linear_stretch_functor
  ...
  const hd_float *in;
  hd_float step;
  linear_stretch_functor(const hd_float *in_, hd_size in_count,
                         hd_size out_count)
...
hd_error linear_stretch(const hd_float *d_in, hd_size in_count, hd_float *d_out,
                        hd_size out_count) {
  using thrust::make_counting_iterator;
  thrust::device_ptr<hd_float> d_out_begin(d_out);
