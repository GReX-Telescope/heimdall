...
#ifndef WRITE_TIME_SERIES_HPP
#define WRITE_TIME_SERIES_HPP
...
// Float data type
void write_host_time_series(const float *data, size_t nsamps, float dt,
                            string filename) {
...
void write_device_time_series(const float *data, size_t nsamps, float dt,
                              string filename) {
  std::vector<float> h_data(nsamps);
  cudaError_t error = cudaMemcpy(&h_data[0], data, nsamps * sizeof(float),
                                 cudaMemcpyDeviceToHost);
  if (error != cudaSuccess) {
    throw std::runtime_error(
        std::string("write_time_series: cudaMemcpy failed: ") +
...
// Integer data type
void write_host_time_series(const unsigned int *data, size_t nsamps,
                            size_t nbits, float dt, string filename) {
...
void write_device_time_series(const unsigned int *data, size_t nsamps,
                              size_t nbits, float dt, string filename) {
