...
struct round_functor {
  ...
  __device__ int operator()(const float &a) const {
...
template <typename T> struct plus_one : public thrust::unary_function<T, T> {
...
public:
  hd_error exec(const hd_float *d_data, hd_size count, hd_float thresh,
    ...
    hd_size giant_data_count =
        thrust::count_if(thrust::cuda::par(g_allocator), d_data_begin,
    ...
    hd_size giant_data_count2 =
        copy_if(thrust::cuda::par(g_allocator),
                make_zip_iterator(
                    make_tuple(d_data_begin, make_counting_iterator(0u))),
                make_zip_iterator(
                    make_tuple(d_data_begin, make_counting_iterator(0u))) +
                    count,
    ...
    thrust::adjacent_difference(
        thrust::cuda::par(g_allocator), d_giant_data_inds.begin(),
    ...
    thrust::inclusive_scan(
        thrust::cuda::par(g_allocator), d_giant_data_segments.begin(),
    ...
    thrust::scatter_if(
        make_transform_iterator(d_giant_data_inds.begin(), plus_one<hd_size>()),
