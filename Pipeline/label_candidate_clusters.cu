...
template <typename T> struct trace_equivalency_chain {
  T *new_labels;
  trace_equivalency_chain(T *new_labels_) : new_labels(new_labels_) {}
  inline /*__host__*/ __device__ void operator()(unsigned int old_label) const {
...
struct cluster_functor {
  ...
  cluster_functor(hd_size count_, const hd_size *d_samp_inds_,
                  const hd_size *d_begins_, const hd_size *d_ends_,
                  const hd_size *d_filters_, const hd_size *d_dms_,
                  hd_size *d_labels_, hd_size time_tol_, hd_size filter_tol_,
                  hd_size dm_tol_)
      : count(count_), d_samp_inds(d_samp_inds_), d_begins(d_begins_),
        d_ends(d_ends_), d_filters(d_filters_), d_dms(d_dms_),
        d_labels(d_labels_), time_tol(time_tol_), filter_tol(filter_tol_),
        dm_tol(dm_tol_) {}

  inline __host__ __device__ void operator()(unsigned int i) {
    ...
    for (unsigned int j = 0; j < count; ++j) {
      ...
      if (are_coincident(samp_i, samp_j, begin_i, begin_j, end_i, end_j,
                         filter_i, filter_j, dm_i, dm_j, time_tol, filter_tol,
                         dm_tol)) {
