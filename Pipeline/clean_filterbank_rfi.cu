...
struct abs_less_than : public thrust::unary_function<T, bool> {
  ...
  inline __host__ __device__ bool operator()(T x) const {
...
struct zap_fb_rfi_functor : public thrust::unary_function<WordType, WordType> {
  ...
  zap_fb_rfi_functor(const int *mask_, const WordType *in_,
      ...
      : mask(mask_), in(in_), stride(stride_), nbits(nbits_),
        bitmask((1 << nbits) - 1), nsamps(nsamps_),
        max_resample_dist(max_resample_dist_) {}
  inline __host__ __device__ WordType operator()(unsigned int i) const {
    ...
    if (mask[t]) {
      ...
      for (int k = 0; k < sizeof(WordType) * 8; k += nbits) {
        unsigned int min_t = t > max_resample_dist ? t - max_resample_dist : 0;
...
struct zap_narrow_rfi_functor
  ...
  zap_narrow_rfi_functor(WordType *data_, const float *baseline_, float thresh_,
                         unsigned int stride_, unsigned int nbits_,
                         unsigned int nchans_, unsigned int max_resample_dist_)
      : data(data_), baseline(baseline_), thresh(thresh_), stride(stride_),
        nbits(nbits_), bitmask((1 << nbits) - 1), nchans(nchans_),
        max_resample_dist(max_resample_dist_),
        chans_per_word(sizeof(WordType) * 8 / nbits) {}

  inline __host__ __device__ WordType sample(unsigned int t,
  ...
  inline __host__ __device__ void operator()(unsigned int i) const {
    // Lift the 1D index into 2D filterbank coords
    unsigned int t = i / stride;
    ...
    for (int k = 0; k < chans_per_word; ++k) {
      ...
      if (fabs(val - baseline[c]) > thresh) {
        ...
        unsigned int min_c = c > max_resample_dist ? c - max_resample_dist : 0;
...
hd_error zap_filterbank_rfi(const int *h_mask, const hd_byte *h_in,
                            ...
                            hd_size max_resample_dist, hd_byte *h_out) {
...
template <typename T> struct is_rfi : public thrust::unary_function<T, bool> {
  T thresh;
...
struct rfi_mask_functor : public thrust::binary_function<T, int, bool> {
  ...
  inline __host__ __device__ bool operator()(T x, int mask) const {
    return (fabs(x) > thresh) || mask;
  }
};

hd_error clean_filterbank_rfi(dedisp_plan main_plan, const hd_byte *h_in,
                              hd_size nsamps, hd_size nbits, hd_byte *h_out,
                              int *h_killmask, hd_float dm, hd_float dt,
                              hd_float baseline_length, hd_float rfi_tol,
                              hd_size rfi_min_beams, hd_size boxcar_max) {
  ...
  if (nbits > 4) {
    ...
    for (hd_size g = 0; g < nsamps; g += nsamps_smooth) {
      ...
      hd_float rms = 0;
      measure_bandpass((hd_byte *)(d_in_ptr + g * stride), nsamps_gulp, nchans,
                       nbits, d_bandpass_ptr, &rms);

      zap_narrow_rfi_functor<WordType> zapit(d_in_ptr, d_bandpass_ptr,
                                             rfi_tol * rms, stride, nbits,
                                             nchans, max_chan_resample_dist);
  ...
  for (hd_size filter_width = 1; filter_width <= boxcar_max;
    ...
    thrust::transform(d_filtered.begin(), d_filtered.end(), norm_val_iter,
                      d_filtered.begin(), thrust::multiplies<hd_float>());
    ...
    mask_filter_plan.prep(d_filtered_rfi_mask_ptr, nsamps_computed, boxcar_max);
  ...
  error = zap_filterbank_rfi(&h_rfi_mask[0], &h_in_copy[0], nsamps_computed,
                             nbits, nchans,
...
hd_error apply_manual_killmasks(dedisp_plan main_plan, int *h_killmask,
