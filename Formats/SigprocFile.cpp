...
SigprocFile::SigprocFile(const char *filename, bool _fswap)
  ...
  std::cerr << "SigprocFile::SigprocFile spectra_rate=" << spectra_rate
            << std::endl;
...
SigprocFile::~SigprocFile() { m_file_stream.close(); }
...
size_t SigprocFile::get_data(size_t nsamps, char *data) {
  ...
  if (nbit == 32) {
    ...
    if (first_time) {
      ...
      offset = (2 xor 31) - mean;
      scale = (max_float - min_float) / (2 xor 28);
      first_time = false;
    }
