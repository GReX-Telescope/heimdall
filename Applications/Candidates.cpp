...
Candidate::Candidate(const char *line, unsigned _beam_number) {
  ...
  if (!iss.eof())
    cout << "Candiate::Candidate too many params on input line [" << line << "]"
         << endl;
}
...
std::ostream &operator<<(std::ostream &os, const Candidate *c) {
  ...
  /*  os << c->snr << " " << c->sample_idx << " " << c->sample_time << " "
     ...
     << c->nbeams << " " << c->beam_mask << " " << c->primary_beam << " "
     << c->max_snr <<" " << c->beam;*/
  os << c->max_snr << " " << c->sample_idx << " " << c->sample_time << " "
     << c->filter << " " << c->dm_trial << " " << c->dm << " " << c->members
     << " " << c->primary_beam << " " << c->beam << " " << c->nbeams << " "
     << c->snr;
  return os;
}
...
CandidateChunk::CandidateChunk(int argc, int optind, char **argv) {
  ...
  for (unsigned int i = 0; i < n_beams; i++) {
    if (verbose)
      cerr << "CandidateChunk::CandidateChunk opening file " << argv[optind + i]
           << endl;
    ...
    if (verbose)
      cerr << "CandidateChunk::CandidateChunk parsed beam number as " << beam
           << endl;
...
int CandidateChunk::addBeam(string _utc_start, string _first_sample_utc,
  ...
  if (n_beams == 0) {
    ...
    if (first_sample != _first_sample)
      cerr << "CandidateChunk::addBeam sample mismatch [" << first_sample
           << " != " << _first_sample << "]" << endl;
  ...
  if (verbose > 1)
    cerr << "CandidateChunk::addBeam resized to " << n_beams
         << " beams with beam " << beam << endl;
  ...
  for (unsigned ievent = 0; ievent < num_events; ievent++) {
    ...
    cerr << "cands[" << ibeam << "][" << ievent << "]=" << cands[ibeam][ievent]
         << endl;
...
unsigned int CandidateChunk::get_n_beams() const { return n_beams; }
...
void CandidateChunk::write_coincident_candidates() {
  ...
  if (verbose)
    cerr << "CandidateChunk::write_coincident_candidates: output_file="
         << filename->c_str() << endl;
  ...
  if (ibeam >= 0 && jcand >= 0) {
    sprintf(ss, "echo %lu-%g-%d-%g | nc -4u -w1 10.10.1.7 11223 &",
            cands[ibeam][jcand]->sample_idx, cands[ibeam][jcand]->snr,
            cands[ibeam][jcand]->filter, cands[ibeam][jcand]->dm);
