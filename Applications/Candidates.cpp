...
std::ostream &operator<<(std::ostream &os, const Candidate *c) {
  os << c->snr << "\t" << c->sample_idx << "\t" << c->sample_time << "\t"
     ...
     << "\t" << c->begin << "\t" << c->end << "\t" << c->nbeams << "\t"
     << c->beam_mask << "\t" << c->primary_beam << "\t" << c->max_snr << "\t"
     << c->beam;
  return os;
}
...
void CandidateChunk::write_coincident_candidates() {
  ...
  if (utc_start == "")
    ...
    if ((stat(utc_start.c_str(), &dir_stat) == 0) &&
      ...
      filename =
          new std::string(utc_start + "/" + first_sample_utc + "_all.cand");
    else {
      cerr << "directory [" << utc_start
           << "] did not exist, not writing candidate file" << endl;
      return;
    }
  }
