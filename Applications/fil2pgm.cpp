...
int main(int argc, char *argv[]) {
  if (argc <= 2) {
    cout << "Usage: " << argv[0] << " infile outfile"
         << " [nsamps] [skip] [tscrunch] [fscrunch]" << endl;
    return 0;
  }
  ...
  if (out_nsamps % tscrunch != 0) {
    ...
    cout << "Warning: Adjusting nsamps to multiple of tscrunch: " << out_nsamps
         << endl;
  }
  ...
  if (header.nchans % fscrunch != 0) {
    cout << "Error: fscrunch (" << fscrunch << ") does not divide nchans ("
         << header.nchans << ")" << endl;
  }
  ...
  // packed_data(header.nchans*header.nsamples/chans_per_word);
  // in_file.read((char*)&packed_data[0],
  // header.nchans*header.nsamples/chans_per_word*sizeof(word_type));
  std::vector<word_type> packed_data(out_nsamps * stride_words);
  in_file.read((char *)&packed_data[0], out_nsamps * stride_bytes);
  in_file.close();

  std::vector<out_type> unpacked_data(header.nchans / fscrunch *
                                      (out_nsamps / tscrunch));
    ...
    for (size_type c = 0; c < (size_type)header.nchans; c += fscrunch) {
      ...
      for (size_type s = 0; s < (size_type)tscrunch; ++s) {
        for (size_type f = 0; f < (size_type)fscrunch; ++f) {
          ...
          word_type x =
              (packed_data[w + (t + s) * (header.nchans / chans_per_word)] >>
               (k * header.nbits)) &
              mask;
        ...
        // tsum += fsum / fscrunch;
        // unpacked_data[c/fscrunch + t*(header.nchans/fscrunch)] = fsum /
        // fscrunch;
      }
      unpacked_data[c / fscrunch +
                    (t / tscrunch) * (header.nchans / fscrunch)] =
  ...
  out_file << "P5 " << header.nchans / fscrunch << " " << out_nsamps / tscrunch
           << " " << 255 << endl;
  out_file.write((char *)&unpacked_data[0],
                 unpacked_data.size() * sizeof(out_type));
  out_file.close();
