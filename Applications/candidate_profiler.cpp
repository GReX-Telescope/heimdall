...
int normalise_series_array(float *out, size_t nsamps, size_t nseries) {
  ...
  for (size_t d = 0; d < nseries; ++d) {
    ...
    */
    ...
    std::copy(&out[i], &out[i + nsamps], buf.begin());
    std::nth_element(buf.begin(), buf.begin() + buf.size() / 4,
    ...
    // Normalise
    std::transform(&out[i], &out[i + nsamps], buf.begin(), abs_deviation(0.));
    std::nth_element(buf.begin(), buf.begin() + buf.size() / 2, buf.end());
    float mad = buf[buf.size() / 2];
    float stddev = mad * 1.4826;
    std::transform(&out[i], &out[i + nsamps], &out[i],
...
int gen_freq_time_plot(string filename, size_t samp, size_t filter, float dm,
                       size_t out_nsamps, size_t fscrunch, float *out,
                       bool verbose) {
  ...
  dedisp_error derror;
  dedisp_plan plan;
  derror = dedisp_create_plan(&plan, header.nchans, header.tsamp, header.fch1,
                              header.foff);
  if (derror != DEDISP_NO_ERROR) {
    cerr << "dedisp_create_plan failed: " << dedisp_get_error_string(derror)
         << endl;
    return -1;
  }
  ...
  if (verbose) {
    ...
    cerr << "Input sample range: " << first_samp << " : "
         << first_samp + in_nsamps << " = " << in_nsamps << " samples" << endl;
    cerr << "DM = " << dm << endl;
  }
  ...
  size_t mask = ((unsigned)1 << header.nbits) -
                1; //(((unsigned)1<<(header.nbits-1))-1)*2+1;
    ...
    for (size_t c = 0; c < (size_t)header.nchans; c += fscrunch) {
      ...
      for (size_t s = 0; s < (size_t)tscrunch; ++s) {
        for (size_t f = 0; f < (size_t)fscrunch; ++f) {
          ...
          word_type x =
              (packed_data[w + (t + s) * nchan_words] >> (k * header.nbits)) &
              mask;
      ...
      // cerr << "out[" << (c/fscrunch*out_nsamps + (t/tscrunch)) << "] = " <<
      // sum / (tscrunch*fscrunch) << endl;
...
int gen_dm_time_plot(string filename, const float *dm_list, size_t dm_count,
                     size_t samp, size_t filter, size_t dm_idx,
                     size_t out_nsamps, size_t out_dm_count, float *out,
  ...
  dedisp_error error;
  dedisp_plan plan;
  error = dedisp_create_plan(&plan, header.nchans, header.tsamp, header.fch1,
                             header.foff);
  if (error != DEDISP_NO_ERROR) {
    cerr << "dedisp_create_plan failed: " << dedisp_get_error_string(error)
         << endl;
    return -1;
  }
  ...
  size_t dm_end = dm_idx < dm_count - ((out_dm_count - 1) / 2 + 1)
                      ? dm_idx + (out_dm_count - 1) / 2 + 1
                      : dm_count;
  if (verbose) {
  ...
  if (error != DEDISP_NO_ERROR) {
    cerr << "dedisp_set_dm_list failed: " << dedisp_get_error_string(error)
         << endl;
    return -1;
  }

  error = dedisp_enable_adaptive_dt(plan, dm_pulse_width, scrunch_tol);
  if (error != DEDISP_NO_ERROR) {
  ...
  */
  ...
  if (!do_filter) {
    ...
    first_samp =
        samp >= out_nsamps / 2 * width ? samp - out_nsamps / 2 * width : 0;
  } else {
    nsamps = out_nsamps + width - 1;
    in_nsamps = out_nsamps + width - 1 + max_delay;
    first_samp = samp > out_nsamps / 2 + width / 2
                     ? samp - out_nsamps / 2 - width / 2
                     : 0;
  }
  ...
  // Dedisperse
  error = dedisp_execute(plan, in_nsamps, (dedisp_byte *)&in[0], header.nbits,
                         (dedisp_byte *)&dedisped[0], out_nbits, 0);
  if (error != DEDISP_NO_ERROR) {
    cerr << "dedisp_execute failed: " << dedisp_get_error_string(error) << endl;
    return -1;
  }
  ...
  if (!do_filter) {
    ...
    for (size_t d = 0; d < out_dm_count; ++d) {
      ...
      std::transform(scanned.begin() + width / 2 + (width - 1) / 2 + 1,
                     scanned.begin() + width / 2 + (width - 1) / 2 + 1 +
                         out_nsamps,
                     scanned.begin() + width / 2 - width / 2,
                     &out[d * out_nsamps], std::minus<float>());
    }
  }
...
// Helper functors
template <typename T> struct min_t : public std::binary_function<T, T, T> {
  T operator()(T a, T b) const { return std::min(a, b); }
};
template <typename T> struct max_t : public std::binary_function<T, T, T> {
  T operator()(T a, T b) const { return std::max(a, b); }
};

void usage(char *binary) {
  cerr << "Usage: " << binary
       << " filename.fil dmlist samp filter dm_idx out_nsamps out_dm_count "
          "fscrunch [do_filter=0]"
       << endl;
  ...
  cerr << "    filter              filter (log2) in which candidate was found"
       << endl;
  cerr << "    dm_idx              line in dmlist which corresponds to nominal "
          "DM for candidate"
       << endl;
  cerr << "    out_nsamps          " << endl;
  cerr << "    out_dm_count        should match the number of lines in dmlist"
       << endl;
  cerr << "    fscrunch            fscrunching factor for Freq vs Time data"
       << endl;
...
int main(int argc, char *argv[]) {
  ...
  std::copy(std::istream_iterator<float>(dm_file),
            std::istream_iterator<float>(), std::back_inserter(dm_list));
  ...
  int error = gen_dm_time_plot(input_name, &dm_list[0], dm_list.size(), samp,
                               filter, dm_idx, out_nsamps, out_dm_count,
                               &dm_time_data[0], do_filter, verbose);
  if (error) {
  ...
  error =
      gen_dm_time_plot(input_name, &dm_list[0], dm_list.size(), samp, filter, 0,
                       out_nsamps, 1, &dm0_time_data[0], do_filter, verbose);
  if (error) {
  ...
  gen_freq_time_plot(input_name, samp, ft_plot_filter, dm_list[dm_idx],
                     out_nsamps, fscrunch, &freq_time_data[0], verbose);
  ...
  for (size_t c = 0; c < out_nchans; ++c) {
    for (size_t t = 0; t < out_nsamps; ++t) {
      ...
      {
        // size_t val;//    = (rawval - minval) / (maxval - minval) *
        // (levels-1);
        /*
        float min_sigma = 0;
        float max_sigma = 8;
  ...
  float minval = std::accumulate(dm_time_data.begin(), dm_time_data.end(),
                                 dm_time_data[0], min_t<float>());
  float maxval = std::accumulate(dm_time_data.begin(), dm_time_data.end(),
                                 dm_time_data[0], max_t<float>());
  size_t max_idx = std::max_element(dm_time_data.begin(), dm_time_data.end()) -
                   dm_time_data.begin();
  if (verbose) {
    ...
    cerr << " Dist from given = " << abs(max_idx % out_nsamps - out_nsamps / 2)
         << endl;
  }
  // TODO: Fix this for DM trial bounds
  size_t dm_idx_start =
      dm_idx > out_dm_count / 2 ? dm_idx - out_dm_count / 2 : 0;
  ...
  for (size_t d = 0; d < std::min(out_dm_count, dm_list.size()); ++d) {
    ...
    snr_dm_file << dd << "\t" << std::setprecision(10) << dm_list[dd] << "\t"
  ...
  for (size_t t = 0; t < out_nsamps; ++t) {
    ...
    snr_time_file << tt << "\t" << std::setprecision(10) << tt << "\t"
  ...
  out_file << "P2 " << out_nsamps << " " << out_dm_count << " " << levels - 1
           << endl;
  for (size_t d = 0; d < out_dm_count; ++d) {
    for (size_t t = 0; t < out_nsamps; ++t) {
      ...
      float min_sigma = 0;
      float max_sigma = 8;
      val = std::min(std::max((rawval - min_sigma) / (max_sigma - min_sigma) *
                                  (levels - 1),
                              0.f),
                     (float)levels - 1);
