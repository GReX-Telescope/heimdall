    ...
    if (nfiles > 0) {
      ...
      while (waiting_for_beams) {
        ...
        try {
          ...
          iss >> utc_start >> first_sample_utc >> first_sample_index >>
              beam_number >> num_events >> ws;

          if (verbose) {
            cerr << "main: UTC_START=" << utc_start
                 << " SAMPLE_UTC=" << first_sample_utc
                 << " SAMPLE_IDX=" << first_sample_index
                 << " BEAM=" << beam_number << " NUM_EVENTS=" << num_events
                 << endl;
          }

          // check first_sample_utc to see if it matches an existing chunk
          if (curr_first_sample_utc.empty() ||
              curr_first_sample_utc.compare(first_sample_utc) != 0) {
            curr_chunk = -1;
            time_t youngest = 0;
            for (unsigned ichunk = 0; ichunk < chunks.size(); ichunk++) {
              // get the relative age between this new beam/chunk and the
              // existing ones
              time_t relative_age =
                  chunks[ichunk]->get_relative_age(first_sample_utc);
            ...
            if (curr_chunk >= 0) {
              if (verbose > 1)
                cerr << "main: found existing chunk for " << first_sample_utc
                     << endl;
            }
            // if not, create a new chunk
            if (curr_chunk == -1) {
              if (verbose > 1)
                cerr << "main: creating new chunk for " << first_sample_utc
                     << endl;
              chunks.push_back(new CandidateChunk());
              curr_chunk = chunks.size() - 1;
            } else
              cerr << "main: new beam " << first_sample_utc
                   << " arrived too late" << endl;
          }
          ...
          if (curr_chunk >= 0) {
            if (verbose > 1)
              cerr << "main: chunks[" << curr_chunk << "]->addBeam("
                   << beam_number << ")" << endl;
            chunks[curr_chunk]->addBeam(utc_start, first_sample_utc,
                                        first_sample_index, beam_number,
                                        num_events, iss);

            // if we have reached the specified number of beams for this
            // chunk, or if more than 3 chunks are stored, dump a chunk
            if (chunks[curr_chunk]->get_n_beams() == total_beams ||
                chunks.size() > max_chunks_to_wait)
              waiting_for_beams = false;
          }
          iss.str("");
          oss.str("");
      ...
      if (!quit_threads) {
        ...
        if (verbose > 1)
          cerr << "main: chunks[" << curr_chunk << "]->compute_coincidence()"
               << endl;
        chunks[curr_chunk]->compute_coincidence();

        // write the output
        if (verbose > 1)
          cerr << "main: chunks[" << curr_chunk
               << "]->write_coincident_candidates()" << endl;
